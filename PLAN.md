...
**后端任务**：
1. 创建扩展功能配置模型（FormExtension.js）
2. 实现扩展功能CRUD接口

**前端任务**：
1. 创建扩展功能设置页面（views/editor/ExtensionSettings.vue）
2. 创建扩展功能Store（stores/extension.js）

**数据库模型**：

```javascript
// FormExtension.js - 表单扩展配置
{
  formId: ObjectId,
  dataTitle: { type: String },           // 数据标题字段标识
  allowComment: { type: Boolean, default: false },
  pushReminder: {
    enabled: Boolean,
    recipients: Array,
    triggerEvents: Array                 // create, update, approve, reject
  },
  submitMessage: { type: String },       // 提交提示信息
  printTemplate: { type: Object },       // 打印模板配置
  smartAssistant: { type: Array },       // 智能助手规则
  dataPush: { type: Array },             // 数据推送配置
  createdAt: { type: Date, default: Date.now },
  updatedAt: { type: Date, default: Date.now }
}
```
